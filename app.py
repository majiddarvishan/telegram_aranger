...
API_ID = int(
    ...
    or st.secrets.get("TELEGRAM_API_ID", 6)
)
API_HASH = (
    os.getenv("TELEGRAM_API_HASH")
    or st.secrets.get("TELEGRAM_API_HASH", "eb06d4abfb49dc3eeb1aeb98ae0f581e")
)

# ---------------------------------------------------------
# 2. Local SQLite Database Setup
# ---------------------------------------------------------
conn = sqlite3.connect("telegram_tags.db", check_same_thread=False)
cursor = conn.cursor()
cursor.execute(
    """
...
async def fetch_saved_messages(
    limit: int = 100, proxy: dict | None = None
) -> list[dict]:
    ...
    async with Client(
        ...
        me = await app.get_me()
        messages = []
        async for message in app.get_chat_history("me", limit=limit):
            content = message.text or message.caption or "[Media / File]"
            messages.append(
                {
                    "id": message.id,
                    "text": content,
...
async def delete_telegram_message(
    message_id: int, proxy: dict | None = None
) -> None:
...
if "messages" not in st.session_state or st.button("🔄 Refresh Messages"):
    with st.spinner("Fetching messages from Telegram..."):
        try:
            st.session_state.messages = run_async(
                fetch_saved_messages(limit=100, proxy=proxy_config)
            )
...
# ---------------------------------------------------------
all_tags = set()
cursor.execute("SELECT tags FROM message_tags")
...
# 10. Search
# ---------------------------------------------------------
search_query = st.text_input("🔍 Search message text:")
...
if "messages" in st.session_state:
    for msg in st.session_state.messages:
        ...
        if selected_tag != "All" and selected_tag not in current_tags:
            continue
        ...
        with st.container():
            ...
            with col_content:
                st.write(text)
                st.caption(f"📅 Date: {msg['date']} | ID: {msg_id}")
                tg_deep_link = f"https://t.me/c/{msg['user_id']}/{msg_id}"
                st.markdown(f"[🔗 Open in Telegram Client]({tg_deep_link})")
            ...
            with col_actions:
                ...
                if st.button("Save Tags", key=f"save_{msg_id}"):
                    save_tags(msg_id, tags_input.split(","))
                ...
                if st.button(
                    ...
                    try:
                        ...
                        st.success("Message deleted from Telegram!")
                        st.rerun()
                    except Exception as e:
